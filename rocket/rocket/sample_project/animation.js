...
var canvas, canvas_size, gl = null, g_addrs,
	movement = vec2(),	thrust = vec3(), 	looking = false, prev_time = 0, animate = true, animation_time = 0, firstFrame = true;
		var gouraud = false, color_normals = false, solid = false;
var rotMatrix, startTime = 0;
...
Animation.prototype.display = function(time)
	{
		...
		var green = new Material( vec4( .0,.8,.3,1 ), .8, .5, .8, 40 ), // Omit the final (string) parameter if you want no texture
			greyPlastic = new Material( vec4( .5,.5,.5,1 ), .2, .8, .5, 20 ),
			earth = new Material( vec4( .5,.5,.5,1 ), .5, 1, .5, 40 ),
		...
		if(this.graphicsState.animation_time > startTime + 5000){
			var ref = new Firebase("https://rocket-project-5debf.firebaseio.com/");

			ref.on("value", function(snapshot) {
			  rotMatrix = snapshot.val();
			  console.log("test");
			  ref.off("value");
			...
			startTime = this.graphicsState.animation_time;
		}

		var rot = rotation(0, 0, 0, 0 );
		if(rotMatrix){
			rot[0] = rotMatrix["row1"].split(',');
			rot[1] = rotMatrix["row2"].split(',');
			rot[2] = rotMatrix["row3"].split(',');
			rot[3] = rotMatrix["row4"].split(',');
			for(var i = 0; i < 4; i++){
				for(var j = 0; j < 4; j++){
					rot[i][j] = parseFloat(rot[i][j]);
				}
			}
		}
		...
		model_transform = mult( model_transform, rot );
		model_transform = mult( model_transform, scale( 1, 1, 20) );
